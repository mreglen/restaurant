...
def ingredient_list(request):
    ...
    controller = IngredientController(request)
    q = (request.GET.get('q') or '').strip()
    try:
        ingredients = controller.search(q) if q else controller.list()
...
def ingredient_detail(request, ingredient_id):
    ...
    try:
        ingredient = IngredientController(request).get(ingredient_id)
...
def ingredient_create(request):
    ...
    if request.method == 'POST':
        ...
        try:
            IngredientController(request).create(data)
...
def ingredient_update(request, ingredient_id):
    ...
    if request.method == 'POST':
        data = {'name': request.POST.get('name')}
        try:
            controller.update(ingredient_id, data)
    ...
    try:
        ingredient = controller.get(ingredient_id)
...
def ingredient_delete(request, ingredient_id):
    ...
    try:
        IngredientController(request).delete(ingredient_id)

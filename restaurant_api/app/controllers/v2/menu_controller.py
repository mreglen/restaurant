...
from app.schemas.v2.menu_schema import MenuCreate, MenuUpdate, MenuRead
...
def create_menu(data: MenuCreate, db: Session) -> MenuRead:
    ...
    return MenuRead(id=menu.id, name=menu.name, dish_ids=item_ids)
...
def get_all_menus(db: Session) -> List[MenuRead]:
    ...
    BUG-005 FIX: Использует правильный параметр 'dish_ids' вместо 'items'
    ...
    return [
        MenuRead(
            id=menu.id,
            name=menu.name,
            dish_ids=[item.dish_id for item in menu.items if item.dish_id is not None]
        )
        for menu in menus
    ]
...
def get_menu_by_id(menu_id: int, db: Session) -> MenuRead:
    ...
    dish_ids = [item.dish_id for item in menu.items]
    return MenuRead(id=menu.id, name=menu.name, dish_ids=dish_ids)

...
def create_order(data: OrderCreate, db: Session) -> OrderRead:
    ...
    BUG-002 FIX: Сначала проверяет все блюда, затем создаёт заказ и позиции 
    в одной транзакции. Это предотвращает создание "мусорных" заказов при ошибках.
    ...
    # BUG-002 FIX: Сначала проверяем ВСЕ блюда перед созданием заказа
    for item in data.items:
        dish = db.query(Dish).filter(Dish.id == item.dish_id).first()
        if not dish:
            raise HTTPException(status_code=404, detail=f"Идентификатор блюда {item.dish_id} не найден")
    
    # Теперь создаём заказ и все позиции в одной транзакции
    ...
    db.commit()  # Единый коммит для всего заказа
    db.refresh(order)

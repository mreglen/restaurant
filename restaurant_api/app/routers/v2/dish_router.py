...
from app.schemas.v2.dish_schema import DishCreate, DishRead, DishUpdate
from app.controllers.v2.dish_controller import (
    create_dish,
    delete_dish,
    get_all_dishes,
    get_dish_by_id,
...
def get_all_dishes_endpoint(db: Session = Depends(get_db)):
    """
    Возвращает список всех блюд. Доступно всем (публичный эндпоинт).
    """
    return get_all_dishes(db)


# BUG-003 FIX: Маршрут /search/ должен быть ДО /{dish_id}
...
def get_dish_endpoint(dish_id: int, db: Session = Depends(get_db)):
    """
    Возвращает данные блюда по ID. Доступно всем (публичный эндпоинт).
    """
